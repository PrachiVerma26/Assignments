...
function JobList() {
    ...
    const fetchJobs = useCallback(async (page = 1) => {
        ...
        setError("");
        try {
            const data = await getJobs({ page, limit: JOBS_PER_PAGE, search: debouncedSearch, location });
            setJobs(data.jobs || []);
            setPagination({ currentPage: page, totalJobs: data.total || 0 });
            if (data.jobs?.length) {
                setLocations(prev => [...new Set([...prev, ...data.jobs.map(j => j.location).filter(Boolean)])]);
            }
        } catch (err) {
            ...
            <Layout>
                <div className="job-list-page">
                    ...
                    <div className="job-list-toolbar">
                        ...
                        <select className="job-location-filter" value={location} onChange={e => setLocation(e.target.value)}>
                    ...
                    <div className="job-table-container">
                        <table className="job-table">
                            ...
                            <tbody>
                                {isLoading ? (
                                    <tr><td colSpan="4" className="job-table-msg">Loading...</td></tr>
                                ) : jobs.length === 0 ? (
                                    <tr><td colSpan="4" className="job-table-msg">No jobs found.</td></tr>
                                ) : jobs.map(job => (
                                    <tr key={job.id}>
                                        <td>{job.title}</td>
                                        <td>{job.location}</td>
