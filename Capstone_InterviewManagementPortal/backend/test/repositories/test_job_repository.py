...
def patch_job_collection(mocker):
    """Helper to patch the jobs collection used by the repository."""
    collection = mocker.Mock()
    mocker.patch("src.repositories.job_repository.get_job_collection", return_value=collection)
    return collection
...
@pytest.mark.asyncio
async def test_get_jobs_with_pagination_success(mocker):
    ...
    result = await repository.get_jobs(page=2, limit=10)
...
@pytest.mark.asyncio
async def test_get_jobs_with_search_success(mocker):
    ...
    result = await repository.get_jobs(page=1, limit=10, search=search_term)
    assert result["jobs"] == []
...
async def test_get_job_by_id_success(mocker):
    ...
    collection.find_one = AsyncMock(return_value=expected_job)
    result = await repository.get_job_by_id(str(job_id))
    assert result == expected_job
    collection.find_one.assert_awaited_once_with({"_id": job_id})
...
async def test_update_job_success(mocker):
    ...
    result = await repository.update_job(str(job_id), job_data)
    assert result.modified_count == 1
    collection.update_one.assert_awaited_once_with({"_id": job_id}, {"$set": job_data})

@pytest.mark.asyncio
async def test_get_jobs_empty_result(mocker):
    ...
    cursor.to_list = AsyncMock(return_value=[])
    collection.find.return_value = cursor
    result = await repository.get_jobs(page=1, limit=10)
    assert result["jobs"] == []
...
@pytest.mark.asyncio
async def test_get_jobs_pagination_calculation(mocker):
    ...
    cursor.to_list = AsyncMock(return_value=[])
    collection.find.return_value = cursor
    result = await repository.get_jobs(page=3, limit=10)
