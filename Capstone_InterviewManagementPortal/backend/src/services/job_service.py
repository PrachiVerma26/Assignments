...
async def create_new_job(payload: CreateJobRequest) -> CreateJobResponse:
    """Create a new job description."""
    app_logger.info("Create job request received for: %s", payload.title)
    
    title = payload.title.strip()
    existing_job = await job_repository.get_job_by_title(title)
    if existing_job:
    ...
    try:
        result = await job_repository.create_job(job_data)
    except PyMongoError:
...
async def get_job_by_id(job_id: str) -> JobDetailResponse:
    """Get job by ID."""
    app_logger.info("Fetching job by ID: %s", job_id)
    job = await _get_job_or_raise(job_id)
...
async def update_job(job_id: str, payload: UpdateJobRequest) -> JobResponse:
    ...
    updated_job =await job_repository.get_job_by_id(job_id)
