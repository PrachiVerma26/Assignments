...
async def get_user_by_id(user_id: str) -> UserResponse:
    ...
    user = await _get_user_or_raise(user_id)
...
async def change_user_status(user_id: str, status: UserStatus) -> SuccessResponse:
    user = await _get_user_or_raise(user_id)
    ...
    if status == UserStatus.ACTIVE and current_status == UserStatus.ACTIVE.value:
        raise UserAlreadyInactiveException("User is already active.")
